...
class PageStore:
    ...
    def __repr__(self):
        ...
        return f"<Pagestore serving '{self.pagedir}': {recall} records in {pages} pages >"
    ...
    def get_pages_between(self,name,idx1,idx2):
        """get pages with begin before ii included"""
        sql="""select * FROM pages WHERE  name = ? AND end >= ? AND begin <= ?
               ORDER BY begin"""
        res = self.db.execute(sql,(name,idx1,idx2)).fetchall()
